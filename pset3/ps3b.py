...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    pylab.plot([p / numTrials for p in results], label = "SimpleVirus")
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    """
    timesteps, resist = 300, 'guttagonol'
    with_res = [0] * timesteps
    no_res = [0] * timesteps
    for _ in range(numTrials):
        ...
        for i in range(timesteps // 2):
            patient.update()
            with_res[i] += patient.getResistPop([resist])
            no_res[i] += patient.getTotalPop()
        patient.addPrescription(resist)
        for i in range(timesteps // 2, timesteps):
            patient.update()
            with_res[i] += patient.getResistPop([resist])
            no_res[i] += patient.getTotalPop()
    pylab.plot([p / numTrials for p in with_res], label = "With Resistances")
    pylab.plot([p / numTrials for p in no_res], label = "Wihtout Resistances")
